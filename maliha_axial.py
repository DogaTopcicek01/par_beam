...
phantom = np.zeros((Nz, Ny_full, Nx_full))
...
# --- FIXED: crop the correct rows for reconstruction ---
img_cropped = img_full[670:770, :]  # rows 720-769 (50 rows)
Ny, Nx = img_cropped.shape

# Assign to phantom
phantom[0] = img_cropped
print("Cropped phantom shape for reconstruction:", phantom.shape)  # should be (1, 50, 746)

# Optional: plot
vmax = np.percentile(phantom, 99.7)
plt.imshow(phantom[0], vmin=0, vmax=vmax, origin='upper')
plt.colorbar()
plt.title('Cropped Axial Slice Phantom')
plt.show()
...
print(f"Sinogram shape: {y.shape} [Nz, N_views_sub, N_det]")
